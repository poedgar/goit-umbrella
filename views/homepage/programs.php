					...
					<div class="grid gap-20 md:gap-8 md:p-8 xl:w-[592px]">
						...
						<div class="hidden md:flex gap-8 h-20">
							<?php foreach ($all_logos as $logo): ?>
								<?php if ($logo): ?>
									<img src="<?= esc_url($logo['url']); ?>" alt="<?= esc_attr($logo['alt']); ?>" class="w-auto h-full">
								<?php endif; ?>
							<?php endforeach; ?>
						</div>
						...
						<?php if (have_rows('sites', $company_id)): ?>
							...
							?>
								<div class="flex flex-col smOnly:items-center gap-5 md:gap-4">
									...
									<!-- Mobile logo -->
									<?php if ($site_logo): ?>
										<img src="<?= esc_url($site_logo['url']); ?>" alt="<?= esc_attr($site_logo['alt']); ?>" class="md:hidden w-auto h-full <?= ($index === 2 ? 'mb-[60px]' : '') ?>">
									<?php endif; ?>
